...
def generate_monthly_report_pdf(doctor_name, month, year, appointments_data, stats):
    ...
    # Header
    page.insert_text((50, 50), "HOSPITAL MANAGEMENT SYSTEM", fontsize=18, color=primary_blue)
    ...
    page.insert_text((50, 130), f"Period: {month_names[month-1]} {year}", fontsize=12, color=light_gray)
    page.insert_text((50, 150), f"Generated: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", 
    ...
    for i, (label, value) in enumerate(stats_info):
        ...
        page.insert_text((x_pos + 10, box_y + 45), str(value), fontsize=20, color=primary_blue)
    ...
    for idx, apt in enumerate(appointments_data[:max_rows]):
        ...
        # Format data
        date_str = apt.get('appointment_date', '').split('T')[0] if apt.get('appointment_date') else 'N/A'
        ...
        page.insert_text((x_positions[1], y_pos), patient_name, fontsize=9, color=dark_gray)
        page.insert_text((x_positions[2], y_pos), status.capitalize(), fontsize=9, color=status_color)
        page.insert_text((x_positions[3], y_pos), diagnosis, fontsize=9, color=light_gray)
        
        y_pos += 18
    
    if len(appointments_data) > max_rows:
        page.insert_text((50, y_pos + 10), 
...
def generate_patient_history_pdf(patient_name, patient_id, appointments_data):
    ...
    # Patient Information
    page.insert_text((50, 110), f"Patient: {patient_name}", fontsize=14, color=dark_gray)
    page.insert_text((50, 130), f"Patient ID: {patient_id}", fontsize=12, color=light_gray)
    page.insert_text((50, 150), f"Report Date: {datetime.now().strftime('%B %d, %Y')}", 
                     fontsize=10, color=light_gray)
    ...
    y_pos = 190
    page.insert_text((50, y_pos), f"Medical History ({len(appointments_data)} visits)", 
    ...
    for idx, apt in enumerate(appointments_data[:8]):  # Limit to first 8
        ...
        page.insert_text((60, y_pos + 20), f"Visit {len(appointments_data) - idx}", 
                        fontsize=11, color=primary_blue)
        page.insert_text((60, y_pos + 35), f"Date: {date_str}", fontsize=9, color=dark_gray)
        page.insert_text((200, y_pos + 35), f"Doctor: {doctor}", fontsize=9, color=dark_gray)
        page.insert_text((60, y_pos + 50), f"Diagnosis: {diagnosis[:50]}", fontsize=9, color=light_gray)
        page.insert_text((60, y_pos + 63), f"Treatment: {treatment[:50]}", fontsize=9, color=light_gray)
